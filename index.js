...
function customAssign(...objects) {
  ...
  for (const key in mergedObject) {
    if (mergedObject.hasOwnProperty(key)) {
      let value = mergedObject[key];
      if (typeof value === "number") {
        value = value.toString();
      }
      mergedObject[key] = value.replace(/ /g, "_");
    }
  }
...
function checkAndPreventExtensions(obj) {
  ...
  const isExtensibleBefore = Object.isExtensible(obj);

  if (isExtensibleBefore) {
    Object.preventExtensions(obj);
  }

  const isExtensibleAfter = Object.isExtensible(obj);

  return isExtensibleAfter;
}
...
function sealAndCheck(obj) {
  ...
  const isSealedAfter = Object.isSealed(obj);

  return isSealedAfter;
}
